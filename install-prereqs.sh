#!/usr/bin/env bash
# install-prereqs.sh — install OctOBX's toolchain prerequisites.
#
# Installs: build tools (build-essential/cmake/make/xxd/python3), the
# Emscripten SDK (user-local, ~/emsdk), Node.js >= 20, and Rust (via rustup,
# user-local) for the gui launcher. Idempotent: every component is checked
# before install, so re-runs are fast and safe.
#
# Supports: Debian/Ubuntu, Fedora/RHEL, Arch, macOS (Homebrew).
# After running this, run ./build.sh to build the WASM engines + app.
#
# Can be curl'd directly:
#   curl -sSf https://raw.githubusercontent.com/maxolgi/OctOBX/master/install-prereqs.sh | bash
# (when piped, the script will not clone the repo for you — do that next)
#
# NOTE: CI requires Node >= 23; local dev works fine on Node 20.
...
MIN_NODE_MAJOR=20    # local dev minimum (CI uses 23)
MIN_RUST_MAJOR=1
MIN_RUST_MINOR=75
...
usage() {
    cat <<'EOF' >&2
install-prereqs.sh — install OctOBX toolchain
...
What gets installed:
  - build-essential / cmake / make / xxd / python3 / curl / git  (system)
  - Emscripten SDK (latest)                                      (~/emsdk)
  - Node.js >= 20 (CI requires 23)                               (system)
  - rustup + stable Rust toolchain >= 1.75 (gui launcher)        (~/.cargo)
...
check_system_pkgs() {
    local missing=0
    for tool in cc c++ make cmake xxd python3 git curl; do
        if ! have "$tool"; then
            warn "missing: $tool"
            missing=1
        fi
...
install_system_pkgs() {
    info "installing system build tools (incl. xxd)"
    case "$PLATFORM_OS-$DISTRO" in
        linux-debian)
            sudo_maybe apt-get update
            sudo_maybe apt-get install -y build-essential cmake make xxd python3 curl git
            ;;
        linux-fedora)
            sudo_maybe dnf install -y gcc gcc-c++ make cmake vim-common python3 curl git
            ;;
        linux-arch)
            sudo_maybe pacman -Sy --noconfirm base-devel cmake make xxd python curl git
            ;;
        macos-)
            if ! have brew; then
            ...
            brew install cmake make vim python3
            ;;
        *)
...
# emsdk_env.sh puts emcc/em++/node-for-emcc on PATH for the current shell.
source_emsdk_env() {
...
install_emsdk() {
    if [[ ! -d "$EMSDK_DIR/.git" ]]; then
        info "cloning emsdk to $EMSDK_DIR (large, ~hundreds of MB)"
        git clone https://github.com/emscripten-core/emsdk.git "$EMSDK_DIR"
    fi
    info "installing + activating latest Emscripten SDK (large download)"
    (cd "$EMSDK_DIR" && ./emsdk install latest && ./emsdk activate latest)
    source_emsdk_env
    if ! have emcc; then
        err "emsdk install finished but emcc is not on PATH"
        err "run 'source $EMSDK_DIR/emsdk_env.sh' and rerun"
        return 1
    fi
    ok "emcc $(emcc --version 2>/dev/null | head -1 | grep -oE '[0-9]+([.][0-9]+)+' | head -1) installed"
}
...
install_node() {
    ...
    case "$PLATFORM_OS-$DISTRO" in
        linux-debian)
            ...
            curl -fsSL "https://deb.nodesource.com/setup_20.x" | sudo_maybe -E bash -
            sudo_maybe apt-get install -y nodejs
            ;;
        linux-fedora)
            info "installing Node.js 20 via NodeSource (dnf)"
            curl -fsSL "https://rpm.nodesource.com/setup_20.x" | sudo_maybe -E bash -
...
print_summary() {
    ...
    if have cc && have make && have cmake && have xxd && have python3; then
        ok "system build tools (cc/make/cmake/xxd/python3)"
    else
        err "system build tools (cc/make/cmake/xxd/python3)"
        fail_count=$((fail_count + 1))
    fi

    if check_emsdk; then
        ok "emcc    $(emcc --version 2>/dev/null | head -1 | grep -oE '[0-9]+([.][0-9]+)+' | head -1)"
    else
        err "emcc    missing (emsdk at $EMSDK_DIR)"
        fail_count=$((fail_count + 1))
    fi
    ...
    if check_node; then
        ...
        err "node    missing or too old (need >= v$MIN_NODE_MAJOR; CI needs 23)"
        fail_count=$((fail_count + 1))
    fi
    ...
    if check_rust; then
        ...
        err "rust    missing or too old (need >= $MIN_RUST_MAJOR.$MIN_RUST_MINOR, gui launcher)"
        fail_count=$((fail_count + 1))
    fi
    ...
    if [[ $fail_count -eq 0 ]]; then
        ...
        if [[ $CHECK_ONLY -eq 0 ]]; then
            echo "  next steps:" >&2
            echo "    git submodule update --init firmware third_party/JUCE third_party/OB-Xf" >&2
            echo "    (cd third_party/OB-Xf && git submodule update --init --depth 1 libs/JUCE libs/sst/sst-basic-blocks libs/sst/sst-cpputils libs/fmt libs/simde)" >&2
            echo "    ./build.sh          # build everything (source emsdk_env.sh first if emcc is not on PATH)" >&2
            echo "    cargo build --release --manifest-path gui/Cargo.toml" >&2
        fi
        return 0
    else
...
main() {
    ...
    # 1. System packages (build tools incl. xxd)
    if ! check_system_pkgs; then
        install_system_pkgs
    else
        ok "system build tools already installed"
    fi
    ...
    if ! check_emsdk; then
        ...
        ok "emcc $(emcc --version 2>/dev/null | head -1 | grep -oE '[0-9]+([.][0-9]+)+' | head -1) already installed"
    fi
